...
- Feature branch commits/pushes: allowed
- Commits on `main`/`master`: blocked unless user explicitly approves (override: `ALLOW_MAIN_BRANCH_COMMIT=1`)
- Pushes on or to `main`/`master`: blocked unless user explicitly approves (same override)
- AI attribution (Co-Authored-By): blocked by commit-msg-check hook
